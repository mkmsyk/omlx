...
## GPU専有での再計測と結論（2026-09-27 00:00〜00:17、`d366c0c`）

9/26 13時〜23時の計測は、動画エンコードの OS サービス（`VTEncoderXPCService`）が GPU 時間の 74〜90% を
使っていた時間帯で、上の表の絶対値は低く出ている。再起動で解消した後、統合版モデルで Lightning MTP を
オン・オフし、同一の翻訳依頼（出力 約615トークン）を交互に2回ずつ流した。

| モデル | 同時1・なし | 同時1・あり | 同時3・なし（1件／合計） | 同時3・あり（1件／合計） |
|---|---:|---:|---:|---:|
| 26B-A4B | 45〜52 | 56〜73 | 約29 ／ 約85 | 約25〜27 ／ 約75（`batch-parked`） |
| 31B | 10.2〜10.6 | 13.0〜15.0 | 7.4〜8.0 ／ 約23 | 7.4〜7.9 ／ 約23（`batch-parked`） |

- 同時1では MTP が効く（26B 約+25%、31B 約+40%）。
- 同時3では複数要求 MTP は通常のまとめ処理を上回らず、制御が自動で通常処理へ戻す。26B の通常処理は
  3行で1ステップ 23ms（合計約130 tok/s 相当）に対し、MTP の1周は約106ms で 3行×約2.2トークン。
  MoE は下書きトークンごとに別の expert を読むため、検証の費用がトークン数にほぼ比例する。
- 31B は、1件＋MTP（約14）より通常処理3件（合計約23）の方が処理量が大きい。Krisis の31B上限を
  1→3 にした（Krisis `6a06886`）。
- 本番の統合版への切替（段階3）は行わない。複数要求 MTP が得になる条件（検証コストが行数に比例しない
  dense モデルかつ同時2〜3で通常処理より速い）は、この機械の Gemma 4 では成立しなかった。
