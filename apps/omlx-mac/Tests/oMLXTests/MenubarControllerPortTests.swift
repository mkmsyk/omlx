...
final class MenubarControllerPortTests: XCTestCase {
    ...
    // MARK: - webAdminURL
    //
    // The "Open Web Dashboard" item opens the Passport-backed browser entry.
    // The action method itself needs a live NSStatusBar, so we test the pure
    // URL builder it delegates to.

    func testWebAdminURLUsesPassportBackedAdminEntryWithoutCredential() throws {
