...
状態：承認済み（2026-09-26）。段階 0（cherry-pick）と段階 2 の 2.1・2.2・2.4・2.5・2.6（合成モデル試験）を実装済み、配備待ち。2.3（行まとめ下書き）は実測後に判断。統合チェックポイントは作成済み（段階 1 の 1.1）、Krisis に試験用カタログ項目を追加済み（1.3）。経過は [作業記録](docs/internal/session-gemma-batch-mtp-20260926.md)。／作成 2026-09-26
...
2.7 **検証用の 6bit 量子化行列積 kernel（2026-09-26 追加・未着手）**

- 実測の結論：複数要求 MTP の合計が伸びない主因は、MLX の 6bit 量子化行列積が M=2〜16 で M にほぼ比例して重くなること（31B MLP down：M=1 433μs → M=12 1,401μs）。検証トークン数は「行数 ×（深さ＋1）」なので、束ねるほど重くなる。詳細は作業記録の「実機計測」。
- 方針：`qwen35_verify_qmm`（MTPLX 由来の split-K / msg kernel、4bit/8bit・M=3〜6）を土台に、6bit のパッキング（4 値 / 3 バイト）と M=2〜16 に拡張する。武装は既存どおり `_call_backbone` の verify forward 中だけ。
- 対象：まず dense の 31B（`nn.QuantizedLinear`）。MoE の 26B の expert 行列積（`gather_qmm`）は別経路なので、31B で効果を確認してから判断する。
- 成功条件：31B 相当の形状で M=4〜16 の時間が M=1 の 1.5 倍以内。31B 実機で同時3件の合計が同時1件の 2 倍以上。数値差は bf16 末尾程度で、greedy 出力は検証済み経路の範囲に収まる。
